...
linkml_meta = LinkMLMeta({'created_by': 'orcid:0000-0002-6095-8718',
     'default_prefix': 'https://example.org/pidinst-foo/',
     'default_range': 'string',
     'description': 'An example of a schema that extends the PIDInst schema.',
     ...
     'title': 'Foo extension to the PIDInst schema'} )
...
class Foo(ConfiguredBaseModel):
    """
    The Foo extension proper. This class should contain all the desired additional slots to represent all the additional metadata.
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'from_schema': 'https://example.org/pidinst-foo'})

    name: str = Field(default=..., description="""An example of an additional metadata slot.""", json_schema_extra = { "linkml_meta": {'domain_of': ['PIDInstIsIdentifiableEntity',
                       'PIDInstRelatedIdentifier',
                       'PIDInstAlternateIdentifier',
                       'Foo']} })
    type: Optional[str] = Field(default=None, description="""Another example of an additional metadata slot.""", json_schema_extra = { "linkml_meta": {'domain_of': ['PIDInstDate',
                       'PIDInstRelatedIdentifier',
                       'PIDInstAlternateIdentifier',
                       'Foo']} })
...
class FooMixin(ConfiguredBaseModel):
    """
    A mixin that wraps the Foo extension proper. Any extension class should be wrapped in such a mixin. This allows a third-party to compose several extensions together (which is useful when one wants to support several extensions at the same time).
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'from_schema': 'https://example.org/pidinst-foo', 'mixin': True})

    foo: Optional[Foo] = Field(default=None, description="""A slot intended to contain an instance of the Foo extension. This is the slot that will be added to the base class.""", json_schema_extra = { "linkml_meta": {'domain_of': ['FooMixin']} })


class FooInstrument(FooMixin, PIDInstInstrument):
    """
    The Instrument class extended with the Foo extension. This class is constructed simply by (1) inheriting from the base Instrument class and (2) adding the `FooMixin` mixin that wraps the Foo extension.
    """
